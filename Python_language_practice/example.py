...
def remove_noise(tweet_token, stop_word=()):
    cleaned_tokens = []
    ...
    for token, tag in pos_tag(tweet_token):
        ...
        if len(token) > 0 and token not in string.punctuation and token.lower() not in stop_word:
            cleaned_tokens.append(token.lower())
    return cleaned_tokens


def get_all_words(cleaned_tokens_list):
    for the_tokens in cleaned_tokens_list:
